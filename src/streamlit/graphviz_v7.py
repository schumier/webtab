...
if show_mode == "Normal network path":
    if filter_mode == "All reachable nodes from first node":
        ...
        # Get distinct categories from df
        all_cats = ["All"] + sorted(set(df["from_cat"]).union(set(df["to_cat"])))
        selected_cat = st.selectbox("Filter by Column Type", all_cats)
        ...
        for first_node in first_nodes:
            ...
            for target in reachable_nodes:
                # Filter target node by selected_cat (skip if not matching, unless "All")
                if selected_cat != "All":
                    # Find the category for the target node from df
                    target_cats = set(df.loc[df["to"] == target, "to_cat"]).union(df.loc[df["from"] == target, "from_cat"])
                    if selected_cat not in target_cats:
                        continue
                try:
                    for path in nx.all_simple_paths(G, source=first_node, target=target):
                        for i in range(len(path)-1):
                            edge_data = G.get_edge_data(path[i], path[i+1])
                            all_edges_in_paths.append((
                                path[i], path[i+1],
                                edge_data["flag"],
                                edge_data["join_type"],
                                edge_data.get("join_key", "")
                            ))
                except nx.NetworkXNoPath:
                    continue
        # Deduplicate edges by source, target, flag, join_type, join_key
        deduped = {}
        ...
        if not all_edges_in_paths:
            ...
            for edge in all_edges_in_paths:
                ...
                edge_title = f"{join_key}" if join_key else ""
                net.add_edge(
                    from_node, to_node,
                    color=color,
                    arrows="to",
                    title=edge_title,  # Tooltip only, not visible label
                    font={"size": 9}
                )
                if flag == "Y":
                    net.add_edge(
            ...
            if filter_mode == "Free text (comma separated)":
                ...
                for edge in all_edges_in_paths:
                    ...
                    net.add_edge(
                        ...
                        color=color,
                        arrows="to",
                        label=edge_title,  # Shows label on graph, not just on hover
                        font={"size": 14, "background": "yellow"}  # font size and background for label
                    )
                    if flag == "Y":
    ...
    if show_mode == "Predecessor node":
        ...
        for last_node in last_nodes:
            ...
            for pred in G.predecessors(last_node):
                ...
                if edge_data:
                    predecessor_edges.add((pred, last_node, edge_data["flag"], edge_data["join_type"], edge_data.get("join_key", "")))
                    used_nodes.add(pred)
            ...
            for target in reachable_nodes:
                try:
                    for path in nx.all_simple_paths(G, source=last_node, target=target):
                        for i in range(len(path)-1):
                            edge_data = G.get_edge_data(path[i], path[i+1])
                            next_edges.add((path[i], path[i+1], edge_data["flag"], edge_data["join_type"], edge_data.get("join_key", "")))
        ...
        if all_edges:
            ...
            for edge in all_edges:
                ...
                net_pred.add_edge(
                    ...
                    color=color,
                    arrows="to",
                    title=edge_title,  # Tooltip for edge
                    font={"size": 9}
                )
                if flag == "Y":
                    net_pred.add_edge(
...
""")
        ...
        # 3. Filter direct targets by selected_cat (skip if not matching, unless "All")
        filtered_direct_targets = set()
        for target in common_direct_targets:
            if selected_cat != "All":
                target_cats = set(df.loc[df["to"] == target, "to_cat"]).union(df.loc[df["from"] == target, "from_cat"])
                if selected_cat not in target_cats:
                    continue
            filtered_direct_targets.add(target)

        # 4. Collect edges from each first node to each filtered direct target
        common_edges = set()
        for first_node in first_nodes:
            for target in filtered_direct_targets:
                edge_data = G.get_edge_data(first_node, target)
                if edge_data:
                    common_edges.add((first_node, target, edge_data["flag"], edge_data["join_type"], edge_data.get("join_key", "")))

        # 5. Collect all nodes used in these edges
        used_nodes = set(first_nodes)
        used_nodes.update(filtered_direct_targets)
